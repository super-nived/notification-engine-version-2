"""Dispatcher — executes scheduled rules based on next_run_at.

Stores next_run_at on the rule itself. No bulk schedule records.
After each execution, advances next_run_at by the rule's frequency.
...
class Dispatcher:
    """Executes scheduled rules based on next_run_at field."""
    ...
    def on_rule_deleted(self, rule: dict) -> None:
        """Nothing to clean up — rule will be deleted."""
        pass
...
def _init_next_run_for_all():
    ...
    try:
        ...
        logger.error("Failed to load rules at startup: %s", exc)
        return

    for rule in rules:
        if rule_is_scheduled(rule) and not rule.get("next_run_at"):
            _set_initial_next_run(rule)
...
def _parse_frequency_minutes(frequency: str) -> int:
...
def _set_initial_next_run(rule: dict) -> None:
    ...
    minutes = _parse_frequency_minutes(
        rule.get("frequency", "Hourly")
    )
    next_time = _next_slot(datetime.now(timezone.utc), minutes)
    _save_next_run(rule, next_time)
...
def _save_next_run(rule: dict, next_time: datetime) -> None:
    """Persist next_run_at to the database."""
    iso = next_time.strftime("%Y-%m-%d %H:%M:%S.000Z")
    try:
        update_rule_next_run(rule["id"], iso)
    except Exception as exc:
        logger.error(
            "Failed to save next_run for '%s': %s",
            rule.get("name", ""),
            exc,
        )
...
def _clear_next_run(rule_id: str) -> None:
    """Clear next_run_at (rule disabled)."""
    try:
...
def _run_due_rules(due: list[dict]):
    """Execute due rules, auto-scale if > threshold."""
...
def _handle_expired(rule: dict) -> None:
    """Disable and clear next_run for expired rules."""
    logger.info("Rule '%s' expired", rule.get("name", ""))
    _safe_disable(rule["id"])
    _clear_next_run(rule["id"])


def _on_success(rule: dict, started_at: str, events: list):
    """Post-execution: advance next_run, log success."""
...
def _safe_disable(rule_id: str):
    try:
        ...
        logger.error("Failed to disable rule: %s", exc)
...
def _safe_update_last_run(rule_id, status, timestamp):
    try:
        ...
        logger.error("Failed to update last_run: %s", exc)
