"""Dispatcher: schedule_records mode — old method (standalone).

Creates bulk schedule records in the schedules collection.
Kept for backward compatibility. Can be removed in the future.

To use: set SCHEDULER_MODE=schedule_records in .env
"""
...
class ScheduleRecordsDispatcher:
    """Old dispatcher that uses bulk schedule records."""
    ...
    def start(self):
        ...
        self._thread.start()
        logger.info("ScheduleRecordsDispatcher started")

    def stop(self):
        self._running = False
        self._wake.set()
        logger.info("ScheduleRecordsDispatcher stopped")
    ...
    def on_rule_disabled(self, rule: dict) -> None:
        from app.db.pb_repositories import (
            skip_pending_schedules_for_rule,
        )
        try:
            skip_pending_schedules_for_rule(rule["id"])
    ...
    def on_rule_deleted(self, rule: dict) -> None:
        from app.db.pb_repositories import (
            delete_schedules_for_rule,
        )
        try:
            delete_schedules_for_rule(rule["id"])
        except Exception as exc:
            logger.error("Failed to delete schedules: %s", exc)
...
def _recover_stale_schedules():
    try:
        ...
        logger.warning("Could not mark stale schedules: %s", exc)
...
def _run_rule(rule, schedule, started_at) -> list[dict]:
...
def _mark_success(schedule, rule, started_at, events):
...
def _mark_failure(schedule, started_at, error):
...
def _log_execution(rule, started_at, status, events_count, error=""):
    try:
        create_execution_log(
            {
